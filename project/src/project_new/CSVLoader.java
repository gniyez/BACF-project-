/**
 * Loads student data from CSV file with error handling
 * Continues processing even if individual lines fail
 * 
 * @param users the list to add loaded students to
                ...
                if (data.length >= 5) {
                    try {
                        ...
                        if (!studentID.matches("^U\\d{7}[A-Z]$")) {
                            System.out.println("Warning: (Line " + lineNumber + "): Invalid student ID format: " + studentID + ". Skipping entry.");
                            errorCount++;
                            continue;
                        }
                        ...
                        //validate year range
                        if(year<1 || year>5){
                            System.out.println("Warning (Line " + lineNumber + "): Invalid year of study: " + year + ". Skipping entry");
                            errorCount++;
                            continue;
                        }
                        ...
                        //added error counts in below cases 
                        
                    } catch (NumberFormatException e) {
                        System.out.println("Invalid year format in student data: " + lineNumber);
                        errorCount++;
                    } catch (Exception e) {
                        System.out.println("Error parsing student data: " + lineNumber + " - " + e.getMessage());
                        errorCount++;
                    }
                } else {
                    System.out.println("Invalid student data format (expected 5 columns, got " + data.length + "): " + lineNumber);
                    errorCount++;
                }
            }
            System.out.println("Successfully loaded " + loadedCount + " students" + (errorCount > 0 ? " ( " + errorCount + " errors skipped)" : ""));
        }
        //added catch
        catch(IOException e){
